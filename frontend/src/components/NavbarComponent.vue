      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0 w-100">
          ...
          <li class="nav-item" v-if="is_login">
            <router-link to="/admin/add" class="nav-link active">Add</router-link>
          </li>
          <li class="nav-item w-100">
            <form class="d-flex" role="search" @submit.prevent="search()">
              <input class="form-control me-2 w-100" type="search"
                placeholder="Explore an endless collection of images..." aria-label="Search" v-model="search_text">
            </form>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    search() {
      router.push("/search/" + this.search_text).then(() => {
